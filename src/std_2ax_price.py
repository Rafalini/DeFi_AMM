...
for df in dfs:
    ...
    if "attacker" not in df["name"]:
        ...
        filtered.loc[condition, 'USDvalue'] = filtered.loc[condition,'USDvalue'] + 0.1
        # condition = filtered['Timestamp']>31
        # filtered.loc[condition, 'USDvalue'] = filtered.loc[condition,'USDvalue'] - 0.035
        # ax1.plot(filtered['Timestamp'], filtered['USDvalue'], marker='o', color=random_blue())
        ax1.plot(filtered['Timestamp'], filtered['USDvalue'], marker='o')
for df in dfs:
...
ax2.plot(frames['time_ora'], frames['sum'], label="Sumaryczna wartość aktywów", linewidth=4)
